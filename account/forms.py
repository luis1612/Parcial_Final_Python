...
class RegistrationForm(UserCreationForm):
    email = forms.EmailField(
        max_length=254, help_text="Necesario. Introduzca una dirección de correo electrónico válida")
    ...
    def clean(self):
        if self.is_valid():
            ...
            if not authenticate(email=email, password=password):
                raise forms.ValidationError("Detalles de inicio de sesión no válidos")
...
class AccountUpdateForm(forms.ModelForm):
    ...
    def clean_email(self):
        ...
        try:
            ...
            return email
        raise forms.ValidationError('Email "%s" Esta en uso.' % account)
    ...
    def clean_username(self):
        ...
        try:
            ...
            return username
        raise forms.ValidationError(
            'Username "%s" Esta en uso.' % username)
